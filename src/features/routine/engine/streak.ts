...
import { parseLocalDateString, isRoutineScheduledForDate, getDaysDiff } from './scheduler';
...
export function isConsecutiveCompletion(
  ...
  const lastDate = parseLocalDateString(lastDateStr);
  const completedDate = parseLocalDateString(completedDateStr);

  const dayDiff = getDaysDiff(lastDate, completedDate);
  ...
  for (let i = 1; i < dayDiff; i++) {
    ...
    if (isRoutineScheduledForDate(routine, checkDate)) {
 ...
 */
export function rebuildStreakFromLogs(
  routine: Routine,
  completedLogs: RoutineLog[]
): StreakResult {
  // Reset streak to base state
  let currentStreak = 0;
  let longestStreak = 0;
  let lastCompletedDate: string | null | undefined = undefined;
