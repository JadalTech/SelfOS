...
const POROSITY_LABELS: Record<HairPorosity, string> = {
  low: 'Low Porosity',
...
const SCALP_TYPE_LABELS: Record<ScalpType, string> = {
  dry: 'Dry Scalp',
  normal: 'Normal Scalp',
  oily: 'Oily Scalp',
...
const DENSITY_LABELS: Record<HairDensity, string> = {
  thin: 'Thin Density',
...
export function mapToHairConditionVM(condition: HairCondition): HairConditionVM {
...
export function mapToHairConditionVMs(conditions: HairCondition[]): HairConditionVM[] {
  return conditions.map(mapToHairConditionVM);
}

export function filterConditionVMs(
  vms: HairConditionVM[],
  ...
  if (searchKeyword && searchKeyword.trim().length > 0) {
    ...
    filtered = filtered.filter(
      (vm) =>
        vm.formattedDate.toLowerCase().includes(q) ||
