...
export const sleepEntryConverter = {
  toFirestore(entry: WithFieldValue<SleepEntry>): DocumentData {
    const quality = entry.quality as any;
    return {
      userId: entry.userId,
      date: entry.date,
      bedtime: entry.bedtime instanceof Date ? Timestamp.fromDate(entry.bedtime) : Timestamp.now(),
      wakeTime: entry.wakeTime instanceof Date ? Timestamp.fromDate(entry.wakeTime) : Timestamp.now(),
      ...
      tags: entry.tags || [],
      createdAt: entry.createdAt instanceof Date ? Timestamp.fromDate(entry.createdAt) : Timestamp.now(),
      updatedAt: Timestamp.now(),
    };
  },
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): SleepEntry {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
      date: data.date || '',
      bedtime: data.bedtime instanceof Timestamp ? data.bedtime.toDate() : new Date(),
      wakeTime: data.wakeTime instanceof Timestamp ? data.wakeTime.toDate() : new Date(),
...
export const sleepScheduleConverter = {
  toFirestore(schedule: WithFieldValue<SleepSchedule>): DocumentData {
    return {
      ...
      createdAt: schedule.createdAt instanceof Date ? Timestamp.fromDate(schedule.createdAt) : Timestamp.now(),
      updatedAt: Timestamp.now(),
    };
  },
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): SleepSchedule {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
...
export const sleepGoalConverter = {
  toFirestore(goal: WithFieldValue<SleepGoal>): DocumentData {
    return {
      userId: goal.userId,
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): SleepGoal {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
...
export const sleepTrendConverter = {
  toFirestore(trend: WithFieldValue<SleepTrend>): DocumentData {
    return {
      userId: trend.userId,
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): SleepTrend {
    const data = snapshot.data(options);
    return {
      userId: data.userId || '',
