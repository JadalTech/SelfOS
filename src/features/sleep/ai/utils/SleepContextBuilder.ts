...
export class SleepContextBuilder {
  /**
  ...
  static buildContext(params: {
    ...
    monthlySummary: MonthlySleepSummary | null;
  }): SleepAIContext {
    const {
      entries,
      schedule,
    ...
    let bedtimeTrend: 'improving' | 'declining' | 'stable' = 'stable';
    let wakeUpTrend: 'improving' | 'declining' | 'stable' = 'stable';
   ...
   */
  private static sanitizeNotes(notes: string): string {
    let sanitized = notes;
