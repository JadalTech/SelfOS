...
export class NutritionAIService {
  ...
  async fetchConversation(userId: string, maxMessages = 50): Promise<NutritionChatMessage[]> {
    const db = this.getDb();
    const ref = collection(db, 'users', userId, 'nutrition_conversations');
  ...
  async saveMessage(userId: string, message: NutritionChatMessage): Promise<void> {
    const db = this.getDb();
    const ref = doc(db, 'users', userId, 'nutrition_conversations', message.id);
    await setDoc(ref, {
  ...
  async saveRecommendations(userId: string, recommendations: AIRecommendation[]): Promise<void> {
    ...
    const promises = recommendations.map((rec) => {
      const ref = doc(db, 'users', userId, 'nutrition_recommendations', rec.id);
      return setDoc(ref, {
        title: rec.title,
        summary: rec.summary,
...
export const nutritionAIService = new NutritionAIService();
