...
import type { Food, DailyNutritionLog, NutritionGoal, NutritionTemplate } from '../types/nutrition.types';

export const nutritionFoodConverter = {
  toFirestore(food: WithFieldValue<Food>): DocumentData {
    return {
      ...
      createdAt: food.createdAt instanceof Date ? Timestamp.fromDate(food.createdAt) : Timestamp.now(),
      updatedAt: Timestamp.now(),
    };
  },
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): Food {
...
export const nutritionLogConverter = {
  toFirestore(log: WithFieldValue<DailyNutritionLog>): DocumentData {
    return {
      userId: log.userId,
      date: log.date,
      meals: log.meals || [],
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): DailyNutritionLog {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
...
export const nutritionGoalConverter = {
  toFirestore(goal: WithFieldValue<NutritionGoal>): DocumentData {
    return {
      userId: goal.userId,
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): NutritionGoal {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
...
export const nutritionTemplateConverter = {
  toFirestore(template: WithFieldValue<NutritionTemplate>): DocumentData {
    return {
  ...
  fromFirestore(snapshot: QueryDocumentSnapshot, options?: SnapshotOptions): NutritionTemplate {
    const data = snapshot.data(options);
    return {
      id: snapshot.id,
      userId: data.userId || '',
