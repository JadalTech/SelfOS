import type { SkinAssessment, SkinAssessmentVM, Severity } from '../types';
import { SKIN_TYPE_OPTIONS, SKIN_CONCERN_OPTIONS } from '../constants/skincare.constants';

export function mapToSkinAssessmentVM(assessment: SkinAssessment): SkinAssessmentVM {
  const typeOption = SKIN_TYPE_OPTIONS.find((t) => t.value === assessment.skinType);
  ...
  const topConcernsFormatted = (assessment.concerns || []).map((concern) => {
    const concernOption = SKIN_CONCERN_OPTIONS.find((c) => c.value === concern);
  ...
  const oilinessLabels: Record<number, string> = {
    1: 'Very Dry',
    2: 'Dry-Normal',
    3: 'Balanced',
    4: 'Oily',
  ...
  const barrierLabels: Record<number, string> = {
    1: 'Severely Damaged',
    2: 'Compromised',
    3: 'Normal',
    4: 'Healthy',
...
export function mapToSkinAssessmentVMs(assessments: SkinAssessment[]): SkinAssessmentVM[] {
