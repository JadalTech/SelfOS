...
class SkincareAIService {
    ...
    async fetchConversation(userId, maxMessages = 50) {
        const db = this.getDb();
        const ref = (0, firestore_1.collection)(db, 'users', userId, 'skin_conversations');
    ...
    async saveMessage(userId, message) {
        const db = this.getDb();
        const ref = (0, firestore_1.doc)(db, 'users', userId, 'skin_conversations', message.id);
    ...
    async saveRecommendations(userId, recommendations) {
        ...
        const promises = recommendations.map((rec) => {
            const ref = (0, firestore_1.doc)(db, 'users', userId, 'skin_recommendations', rec.id);
