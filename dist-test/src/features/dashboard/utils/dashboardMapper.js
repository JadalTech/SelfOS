...
exports.mapGreeting = mapGreeting;
exports.mapTodayProgress = mapTodayProgress;
exports.mapTodayRoutines = mapTodayRoutines;
exports.mapStats = mapStats;
exports.mapWeeklyProgress = mapWeeklyProgress;
exports.mapRecentActivities = mapRecentActivities;
exports.buildDashboardViewModel = buildDashboardViewModel;
const scheduler_1 = require("../../routine/engine/scheduler");
...
function mapGreeting(user, referenceDate = new Date()) {
...
function mapTodayProgress(routines, logs, referenceDate = new Date()) {
    const todayStr = toDateString(referenceDate);
    const scheduledToday = routines.filter((r) => (0, scheduler_1.isRoutineScheduledForDate)(r, referenceDate));
    const scheduledCount = scheduledToday.length;
    let completedCount = 0;
    for (const routine of scheduledToday) {
        const status = (0, completion_1.getRoutineStatusForDate)(routine, logs, todayStr, referenceDate);
        if (status === 'completed') {
            completedCount++;
        }
    }
    const percentage = scheduledCount > 0 ? Math.round((completedCount / scheduledCount) * 100) : 0;
    const statusText = scheduledCount === 0
...
function mapTodayRoutines(routines, logs, referenceDate = new Date()) {
    const todayStr = toDateString(referenceDate);
    const scheduledToday = routines.filter((r) => (0, scheduler_1.isRoutineScheduledForDate)(r, referenceDate));
...
function mapStats(routines, logs) {
    ...
    for (const routine of activeRoutines) {
        const stats = (0, completion_1.calculateCompletionRate)(routine, logs, startStr, endStr);
        total30DayCompleted += stats.completed;
        total30DayScheduled += stats.scheduled;
    }
    const completionRate30Days = total30DayScheduled > 0 ? Math.round((total30DayCompleted / total30DayScheduled) * 100) : 0;
...
function mapWeeklyProgress(routines, logs, referenceDate = new Date()) {
    const days = [];
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        for (const routine of routines) {
            if ((0, scheduler_1.isRoutineScheduledForDate)(routine, d)) {
                isScheduled = true;
                const status = (0, completion_1.getRoutineStatusForDate)(routine, logs, dateStr, d);
                if (status === 'completed') {
                    isCompleted = true;
                }
            }
...
function mapRecentActivities(routines, logs, limit = 5) {
 ...
 */
function buildDashboardViewModel(user, routines, logs, referenceDate = new Date()) {
    ...
    return {
        ...
        weekly,
        recentActivities,
        modules: module_registry_1.REGISTERED_MODULES,
