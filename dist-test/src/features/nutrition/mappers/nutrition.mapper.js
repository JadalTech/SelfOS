...
exports.mapToFoodVM = mapToFoodVM;
exports.mapToFoodEntryVM = mapToFoodEntryVM;
exports.mapToMealVM = mapToMealVM;
exports.mapToDailyNutritionVM = mapToDailyNutritionVM;
exports.mapToGoalVM = mapToGoalVM;
exports.mapToTemplateVM = mapToTemplateVM;
exports.mapToFoodVMs = mapToFoodVMs;
exports.mapToDailyNutritionVMs = mapToDailyNutritionVMs;
exports.mapToGoalVMs = mapToGoalVMs;
exports.mapToTemplateVMs = mapToTemplateVMs;
const nutrition_constants_1 = require("../constants/nutrition.constants");
function mapToFoodVM(food) {
    const categoryOpt = nutrition_constants_1.FOOD_CATEGORY_OPTIONS.find((c) => c.value === food.category);
    const categoryLabel = categoryOpt ? categoryOpt.label : 'Custom';
    const unitOpt = nutrition_constants_1.SERVING_UNIT_OPTIONS.find((u) => u.value === food.servingUnit);
...
function mapToFoodEntryVM(entry) {
...
function mapToMealVM(meal) {
    const mealOpt = nutrition_constants_1.MEAL_TYPE_OPTIONS.find((m) => m.value === meal.mealType);
...
function mapToDailyNutritionVM(log) {
...
function mapToTemplateVM(template) {
    const mealOpt = nutrition_constants_1.MEAL_TYPE_OPTIONS.find((m) => m.value === template.mealType);
    const mealTypeLabel = mealOpt ? mealOpt.label : 'Meal';
    let totalCalories = 0;
...
function mapToFoodVMs(foods) {
    return foods.map(mapToFoodVM);
}
function mapToDailyNutritionVMs(logs) {
    return logs.map(mapToDailyNutritionVM);
}
function mapToGoalVMs(goals) {
    return goals.map(mapToGoalVM);
}
function mapToTemplateVMs(templates) {
