...
exports.normalizeFirebaseError = normalizeFirebaseError;
...
function normalizeFirebaseError(error) {
    ...
    if (isFirebaseError(error)) {
        ...
        if (mapped) {
            return new AppError_1.AppError(mapped.code, mapped.message, { originalError: error });
        }
        // Unknown Firebase error code — wrap generically
        return new AppError_1.AppError('FIREBASE_ERROR', error.message || 'A Firebase error occurred.', {
            originalError: error,
        });
    }
...
function normalizeError(error) {
    ...
    if (error instanceof Error) {
        ...
        if (isNetworkError(error)) {
            return AppError_1.AppError.network();
        }
        return new AppError_1.AppError('UNKNOWN_ERROR', error.message, { originalError: error });
    }
    if (typeof error === 'string') {
        return new AppError_1.AppError('UNKNOWN_ERROR', error);
    }
    return AppError_1.AppError.unknown(error);
}
/**
