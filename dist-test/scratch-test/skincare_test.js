...
const skincare_validation_1 = require("../src/features/skincare/validation/skincare.validation");
const skincareEngine_1 = require("../src/features/skincare/engine/skincareEngine");
const skincareAnalytics_1 = require("../src/features/skincare/analytics/utils/skincareAnalytics");
const SkinAIContextBuilder_1 = require("../src/features/skincare/ai/utils/SkinAIContextBuilder");
const providerFactory_1 = require("../src/features/skincare/ai/providers/providerFactory");
const ruleRecommendationEngine_1 = require("../src/features/skincare/ai/engine/ruleRecommendationEngine");
const requestManager_1 = require("../src/features/skincare/ai/utils/requestManager");
...
const validProduct = skincare_validation_1.skincareProductSchema.parse({
...
const validRoutine = skincare_validation_1.skincareRoutineSchema.parse({
...
const validAssessment = skincare_validation_1.skinAssessmentSchema.parse({
    recordDate: '2026-07-24',
    skinType: 'combination',
    concerns: ['acne'],
    severityMap: { acne: 2 },
...
const score = (0, skincareEngine_1.calculateSkinHealthScore)({
    hydrationLevel: 5,
...
const expiryInfo = (0, skincareEngine_1.calculateProductExpiry)(new Date('2026-01-01'), 12);
assert(!expiryInfo.isExpired, 'Expiry calculation correct for non-expired product');
const consistency = (0, skincareEngine_1.calculateConsistencyScore)([mockLog], 7);
...
const avgScore = (0, skincareAnalytics_1.calculateAverageHealthScore)([mockAssessment, mockPastAssessment]);
assert(avgScore === 7.1, 'Average health score calculation correct');
const analyticsVM = (0, skincareAnalytics_1.buildSkincareAnalyticsVM)({
...
const aiContext = SkinAIContextBuilder_1.SkinAIContextBuilder.buildContext({
...
const mockProvider = providerFactory_1.SkinAIProviderFactory.getProvider('mock');
assert(mockProvider.name === 'mock', 'Provider factory returns mock provider');
const heuristicProvider = providerFactory_1.SkinAIProviderFactory.getProvider('heuristic');
...
const stage1Recs = ruleRecommendationEngine_1.RuleRecommendationEngine.detectRecommendations(aiContext);
...
const cooldownOk = requestManager_1.aiRequestManager.checkCooldown('test_user_1', 100);
assert(cooldownOk === true, 'Request manager allows initial request');
const cooldownBlocked = requestManager_1.aiRequestManager.checkCooldown('test_user_1', 1000);
