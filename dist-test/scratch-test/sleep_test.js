 ...
 */
const SleepEngine = __importStar(require("../src/features/sleep/engine/sleepEngine"));
const sleep_validation_1 = require("../src/features/sleep/validation/sleep.validation");
const converters_1 = require("../src/features/sleep/firestore/converters");
const sleepRecovery_service_1 = require("../src/features/sleep/services/sleepRecovery.service");
const sleepAnalytics_service_1 = require("../src/features/sleep/services/sleepAnalytics.service");
...
const parsedEntry = sleep_validation_1.sleepEntrySchema.parse(entryInput);
assert(parsedEntry.date === '2026-07-27', 'Sleep Entry parsed successfully');
// 2.2 Invalid Sleep Entry (wakeTime <= bedtime)
try {
    sleep_validation_1.sleepEntrySchema.parse({
        ...entryInput,
...
try {
    sleep_validation_1.sleepScheduleSchema.parse({
        ...mockSchedule,
...
const recoveryRes = sleepRecovery_service_1.sleepRecoveryService.calculateRecovery('u1', mockEntry, mockSchedule, 60, 85);
...
const recoveryHighDebt = sleepRecovery_service_1.sleepRecoveryService.calculateRecovery('u1', mockEntry, mockSchedule, 360, 85);
...
const weeklySummary = sleepAnalytics_service_1.sleepAnalyticsService.calculateWeeklySummary('u1', mockPastEntries, mockSchedule, [mockGoalDuration, mockGoalConsistency], '2026-07-20', '2026-07-27');
...
const featureAnalytics = sleepAnalytics_service_1.sleepAnalyticsService.mapToFeatureAnalytics('u1', mockPastEntries, mockSchedule, [mockGoalDuration, mockGoalConsistency]);
...
const firestoreDoc = converters_1.sleepEntryConverter.toFirestore(mockPastEntries[0]);
...
const backToDomain = converters_1.sleepEntryConverter.fromFirestore({
    id: 'e1',
    data: () => firestoreDoc,
});
...
const failingSleepRepo = new sleep_repository_1.SleepRepository(mockFailingService);
const failingScheduleRepo = new sleep_repository_1.SleepScheduleRepository(mockFailingService);
