...
import {
  ...
  routineFormSchema
} from '../src/features/routine/index';
...
assert(isRoutineScheduledForDate(dailyRoutine, new Date(2026, 6, 20)), 'Should be scheduled on start date');
// 1 day after: not scheduled
assert(!isRoutineScheduledForDate(dailyRoutine, new Date(2026, 6, 21)), 'Should not be scheduled 1 day after');
// 2 days after: scheduled
assert(isRoutineScheduledForDate(dailyRoutine, new Date(2026, 6, 22)), 'Should be scheduled 2 days after');
// Before start date: not scheduled
assert(!isRoutineScheduledForDate(dailyRoutine, new Date(2026, 6, 19)), 'Should not be scheduled before start date');
...
assert(isRoutineScheduledForDate(dailyWithEnd, new Date(2026, 6, 22)), 'Should be scheduled on end date');
assert(!isRoutineScheduledForDate(dailyWithEnd, new Date(2026, 6, 23)), 'Should not be scheduled after end date');
...
assert(isRoutineScheduledForDate(weeklyRoutine, new Date(2026, 6, 20)), 'Scheduled on week 0 Monday');
// July 22, 2026 (Wed): scheduled (week 0)
assert(isRoutineScheduledForDate(weeklyRoutine, new Date(2026, 6, 22)), 'Scheduled on week 0 Wednesday');
// July 24, 2026 (Fri): not scheduled
assert(!isRoutineScheduledForDate(weeklyRoutine, new Date(2026, 6, 24)), 'Not scheduled on week 0 Friday');
// July 27, 2026 (Mon of next week): not scheduled because interval = 2 (week 1)
assert(!isRoutineScheduledForDate(weeklyRoutine, new Date(2026, 6, 27)), 'Not scheduled on week 1 Monday (interval 2)');
// Aug 3, 2026 (Mon of week 2): scheduled
assert(isRoutineScheduledForDate(weeklyRoutine, new Date(2026, 7, 3)), 'Scheduled on week 2 Monday');
...
// July 15: scheduled
assert(isRoutineScheduledForDate(monthlyRoutine, new Date(2026, 6, 15)), 'Scheduled on July 15');
// July 16: not scheduled
assert(!isRoutineScheduledForDate(monthlyRoutine, new Date(2026, 6, 16)), 'Not scheduled on July 16');
// July 30: scheduled
assert(isRoutineScheduledForDate(monthlyRoutine, new Date(2026, 6, 30)), 'Scheduled on July 30');
...
let update = calculateCompletionStreak(streakRoutine, '2026-07-20');
...
update = calculateCompletionStreak(streakRoutine, '2026-07-21');
...
streakRoutine = { ...streakRoutine, ...update };
update = calculateCompletionStreak(streakRoutine, '2026-07-23');
...
let rebuilt = rebuildStreakFromLogs(baseRoutine, logs);
...
rebuilt = rebuildStreakFromLogs(baseRoutine, undoLogs);
...
// July 20: completed
assert(getRoutineStatusForDate(auditRoutine, testLogs, '2026-07-20', new Date(Date.UTC(2026, 6, 23))) === 'completed', 'Audit July 20 completed');
// July 21: skipped
assert(getRoutineStatusForDate(auditRoutine, testLogs, '2026-07-21', new Date(Date.UTC(2026, 6, 23))) === 'skipped', 'Audit July 21 skipped');
// July 22: missed (past date relative to ref July 23, scheduled, no logs)
assert(getRoutineStatusForDate(auditRoutine, testLogs, '2026-07-22', new Date(Date.UTC(2026, 6, 23))) === 'missed', 'Audit July 22 missed');
// July 24: pending (future date relative to ref July 23)
assert(getRoutineStatusForDate(auditRoutine, testLogs, '2026-07-24', new Date(Date.UTC(2026, 6, 23))) === 'pending', 'Audit July 24 pending');
...
const parseResult = routineFormSchema.safeParse(validForm);
...
const invalidResult = routineFormSchema.safeParse(invalidForm);
