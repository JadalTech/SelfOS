 ...
 */
import { SleepPromptBuilder } from '../src/features/sleep/ai/prompts/SleepPromptBuilder';
import { SleepContextBuilder } from '../src/features/sleep/ai/utils/SleepContextBuilder';
import { FallbackHeuristicSleepAIProvider } from '../src/features/sleep/ai/providers/FallbackHeuristicSleepAIProvider';
import { SleepAIProviderFactory } from '../src/features/sleep/ai/providers/providerFactory';
import { RequestManager } from '../src/features/sleep/ai/utils/requestManager';
import { SleepAIRepository } from '../src/features/sleep/ai/repository/SleepAIRepository';
import { SleepEntry, SleepSchedule, SleepGoal } from '../src/features/sleep/types/sleep.types';
import type { SleepAIContext } from '../src/features/sleep/ai/types/sleepAI.types';
...
} from '../src/features/sleep/repository/sleep.repository';
...
async function runTests() {
  ...
  const contextPrompt = SleepPromptBuilder.buildContextSection(sampleContext);
  ...
  const context = SleepContextBuilder.buildContext({
    entries: mockEntries,
    goals: mockGoals,
  ...
  assert(!(context as any).id, 'Database identifier should be stripped');
  assert(!(context.recentGoals[0] as any).id, 'Goal DB ID should be stripped');
  assert(!(context.schedule as any).id, 'Schedule DB ID should be stripped');
  ...
  const provider = new FallbackHeuristicSleepAIProvider();

  // Test chronic sleep debt rule
  const debtContext: SleepAIContext = {
    ...sampleContext,
  ...
  const recoveryContext: SleepAIContext = {
    ...sampleContext,
    recoveryScore: 40,
  };
  ...
  const jetlagContext: SleepAIContext = {
    ...sampleContext,
    schedule: {
      targetDurationMinutes: 480,
  ...
  const consistencyContext: SleepAIContext = {
    ...sampleContext,
    consistencyScore: 50,
  };
  ...
  const mockProv1 = SleepAIProviderFactory.getProvider('mock');
  const mockProv2 = SleepAIProviderFactory.getProvider('mock');
  ...
  const heurProv = SleepAIProviderFactory.getProvider('heuristic');
  ...
  try {
    ...
    assert(err instanceof AppError && err.code === 'AI_COOLDOWN', 'Should throw Rate Limit AppError');
  }
  ...
  const repo = new SleepAIRepository(mockService as any, manager);

  // Set mock database data mocks
  // Mocking imports of repository functions
  (sleepRepository.fetchEntries as any) = async () => ({ success: true, data: mockEntries });
  (sleepScheduleRepository.fetchSchedules as any) = async () => ({ success: true, data: [mockSchedule] });
  (sleepGoalRepository.fetchGoals as any) = async () => ({ success: true, data: mockGoals });
